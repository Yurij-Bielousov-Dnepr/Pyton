...
def SortQuick(sorting_list, leftInd=0, rightInd=0):
    sorting_list_tmp=sorting_list.copy()
    if rightInd == 0:
        rightInd = len(sorting_list_tmp)-1
    i = leftInd
    j = rightInd
    pivot = sorting_list_tmp[leftInd + (rightInd - leftInd) // 2]
    while i <= j:
        while sorting_list_tmp[i] < pivot:
            i+=1
        while sorting_list_tmp[j] > pivot:
            j-=1
        if i <= j:
            sorting_list_tmp[i], sorting_list_tmp[j] = sorting_list_tmp[j], sorting_list_tmp[i]
            i+=1
            j-=1
    if leftInd < j:
        SortQuick(sorting_list_tmp, leftInd, j)
    if i < rightInd:
        SortQuick(sorting_list_tmp, i, rightInd)
...
print("Сортировка объединением, лучший случай")
print("Исходные данные       ",list_best)
print("Отсортированные данные",SortQuick(list_best, metrics[12]))
print("При сортировке сделано ",metrics[12][0],"проходов и замена сделана",metrics[12][1],"раз")

metrics.append([0,0])
print("Сортировка объединением, худший случай")
print("Исходные данные       ",list_bad)
print("Отсортированные данные",SortQuick(list_bad, metrics[13]))
print("При сортировке сделано ",metrics[13][0],"проходов и замена сделана",metrics[13][1],"раз")

metrics.append([0,0])
print("Сортировка объединением, средний случай")
print("Исходные данные       ",list_normal)
print("Отсортированные данные",SortQuick(list_normal, metrics[14]))
print("При сортировке сделано ",metrics[14][0],"проходов и замена сделана",metrics[14][1],"раз")
...
print("Быстрая сортировка лучший случай n=",metrics[12][0],"p=",metrics[12][1],"худший случай n=",metrics[13][0],"p=",metrics[13][1],"средний случай n=",metrics[14][0],"p=",metrics[14][1])
...
dict_sort_n_best = dict(zip(["Пузырьки ","Вставками ","Шелла ","Обьединением ", "Быстрая"], [(metrics[0][0]), (metrics[3][0]),(metrics[6][0]), (metrics[9][0]), (metrics[12][0])]))
dict_sort_n_bad = dict(zip(["Пузырьки ","Вставками ","Шелла ","Обьединением ", "Быстрая "], [(metrics[1][0]), (metrics[4][0]),(metrics[7][0]), (metrics[10][0]), (metrics[13][0])]))
dict_sort_n_norm = dict(zip(["Пузырьки ","Вставками ","Шелла ","Обьединением ", "Быстрая "], [(metrics[2][0]), (metrics[5][0]),(metrics[8][0]), (metrics[11][0]), (metrics[14][0])]))
dict_sort_p_best = dict(zip(["Пузырьки ","Вставками ","Шелла ","Обьединением ","Быстрая "], [(metrics[0][1]), (metrics[3][1]),(metrics[6][1]), (metrics[9][1]), (metrics[12][1])]))
dict_sort_p_bad = dict(zip(["Пузырьки ","Вставками ","Шелла ","Обьединением ","Быстрая "], [(metrics[1][1]), (metrics[4][1]),(metrics[7][1]), (metrics[10][1]), (metrics[13][1])]))
dict_sort_p_norm = dict(zip(["Пузырьки ","Вставками ","Шелла ","Обьединением ","Быстрая "], [(metrics[2][1]), (metrics[5][1]),(metrics[8][1]), (metrics[11][1]), (metrics[14][1])]))
